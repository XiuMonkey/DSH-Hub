...
// ------------------------------------------------------------------
// 获取模型：llm/discoverModels 的回包与请求
// ------------------------------------------------------------------

void TestModelSelection::discoveredModelsParsed()
{
	// 贴近期端点会回的形状：多数只给 id，少数带展示名与容量
	const QJsonValue raw = QJsonDocument::fromJson(R"([
		{ "id": "deepseek-v4-flash" },
		{ "id": "Qwen/Qwen-Image", "name": "Qwen Image", "contextWindow": 131072, "maxTokens": 8192 }
	])").array();

	const QVector<DiscoveredModel> models = ModelSelectionService::parseDiscoveredModels(raw);
	QCOMPARE(models.size(), 2);

	// 只有 id：其余字段保持“没有”，不能编出 0 来冒充公布过的容量
	QCOMPARE(models.at(0).id, QStringLiteral("deepseek-v4-flash"));
	QVERIFY(models.at(0).name.isEmpty());
	QVERIFY(!models.at(0).hasContextWindow);
	QVERIFY(!models.at(0).hasMaxTokens);

	QCOMPARE(models.at(1).id, QStringLiteral("Qwen/Qwen-Image"));
	QCOMPARE(models.at(1).name, QStringLiteral("Qwen Image"));
	QVERIFY(models.at(1).hasContextWindow);
	QCOMPARE(models.at(1).contextWindow, 131072);
	QVERIFY(models.at(1).hasMaxTokens);
	QCOMPARE(models.at(1).maxTokens, 8192);
}

void TestModelSelection::discoveredModelsSkipsRowsWithoutId()
{
	// 一行的毛病不该毁掉整份清单；非数组（服务端报错或空值）也不该崩
	const QVector<DiscoveredModel> models = ModelSelectionService::parseDiscoveredModels(
		QJsonDocument::fromJson(R"([ { "name": "no id" }, { "id": "" }, { "id": "kept" } ])").array());

	QCOMPARE(models.size(), 1);
	QCOMPARE(models.at(0).id, QStringLiteral("kept"));

	QVERIFY(ModelSelectionService::parseDiscoveredModels(QJsonValue(QStringLiteral("nope"))).isEmpty());
	QVERIFY(ModelSelectionService::parseDiscoveredModels(QJsonValue()).isEmpty());
	QVERIFY(ModelSelectionService::parseDiscoveredModels(QJsonValue(QJsonArray())).isEmpty());
}

void TestModelSelection::discoveryRequestCarriesRouteAndProfile()
{
	const QVector<ConfigurableProvider> providers = parseProvidersJson(kProvidersJson);
	bool writable = false;
	const QVector<SettingsNamespace> namespaces =
		ModelSelectionService::parseNamespaces(parseObject(kNamespacesJson), &writable);

	// pi-ai 路由：命名路由 + profile 里的端点；profile 没写 api 就不带这个键
	const QJsonObject siliconflow = ModelSelectionService::buildDiscoveryRequest(
		providers.at(1), &namespaces.at(1));
	QCOMPARE(siliconflow.value(QStringLiteral("provider")).toString(), QStringLiteral("siliconflow-cn"));
	QCOMPARE(siliconflow.value(QStringLiteral("baseURL")).toString(),
		QStringLiteral("https://api.siliconflow.cn/v1"));
	QVERIFY(!siliconflow.contains(QStringLiteral("api")));
	QVERIFY(!siliconflow.contains(QStringLiteral("apiKey")));

	// deepseek 系：profile 整节就是路由本身（settingsPath 为空），没有端点可报
	const QJsonObject deepseek = ModelSelectionService::buildDiscoveryRequest(
		providers.at(0), &namespaces.at(0));
	QCOMPARE(deepseek.value(QStringLiteral("provider")).toString(), QStringLiteral("deepseek-official"));
	QVERIFY(!deepseek.contains(QStringLiteral("baseURL")));

	// 表单里刚敲进去的 key 该赢过服务端存好的凭据：填了才带，留空一律不带
	const QJsonObject withKey = ModelSelectionService::buildDiscoveryRequest(
		providers.at(1), &namespaces.at(1), QStringLiteral("  sk-typed  "));
	QCOMPARE(withKey.value(QStringLiteral("apiKey")).toString(), QStringLiteral("sk-typed"));

	const QJsonObject blankKey = ModelSelectionService::buildDiscoveryRequest(
		providers.at(1), &namespaces.at(1), QStringLiteral("   "));
	QVERIFY(!blankKey.contains(QStringLiteral("apiKey")));

	// 读不到命名空间时仍要报出路由：让适配器用它自己的目录回答
	const QJsonObject noNamespace =
		ModelSelectionService::buildDiscoveryRequest(providers.at(1), nullptr);
	QCOMPARE(noNamespace.size(), 1);
	QCOMPARE(noNamespace.value(QStringLiteral("provider")).toString(), QStringLiteral("siliconflow-cn"));
}