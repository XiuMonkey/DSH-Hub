...
// 读取 JSON5 格式的 DLL 描述文件（regulation.json5）并按描述调用扩展 DLL 的导出函数：json 风格（const char* 出参返回 JSON）与 native 风格（由 Thunk 按真实 C 签名调用）都由本类负责。
// json 风格桥接签名：int|void Func(const char* argsJson, char** resultJson)；string Func(const char* argsJson)（返回 const char*，内容是 JSON）。工具 args 先序列化成 JSON 字符串再传给 DLL。
// 结果内存契约：ReturnType=string 且 DLL 导出了 ClearMem 时，返回值是 DLL malloc 的堆内存，本类读取后立即调 ClearMem 归还；无 ClearMem 的老扩展保持“拷走即用、不释放”。
...
public:
	...
	struct FunctionSpec
	{
		...
		// loadDescriptor 解析后填充：LoadingSource 相对本描述文件所在目录解析出的规范化绝对 DLL 路径；调用期一律按此绝对路径查找/加载库，避免多扩展同名 main.dll 按短文件名缓存导致的串库。
	...
	// 按 tool 名调用 DLL 工具；args 会转成 JSON 字符串传给 DLL。工具的 interfaceType 为 "com" 时委托给 ComCaller（comcall::invoke）。
...
private:
	...
	// 一个已加载扩展 = 扩展名 + 一份描述符。扩展名取描述文件所在目录的 basename（安装布局 extensions/<name>/regulation.json5），与扩展管理/移除流程同源；同名扩展不允许重复加载（先移除再重装）。
	...
	// 一个已加载的 DLL；按规范化绝对路径缓存，多扩展同名 main.dll 互不冲突。线程模型：callTool 在 Worker 线程执行，同一 DLL 的调用经 runMutex 串行（旧扩展可能带 static 缓冲/非重入代码）、不同 DLL 之间并行；inFlight/drained 用于卸载/移除时等待在途调用结束，防止卸载中的 QLibrary 被使用。
	...
	// 返回 fn 对应 DLL 的已加载 QLibrary；未加载则按 resolvedDllPath 加载。失败时返回 nullptr 并设置 m_errorString。
	QLibrary* libraryForPath(const QString& absoluteDllPath);

	// 假定调用方已持有 m_mutex：按规范化绝对路径取运行时条目，缺失则加载。失败时返回 nullptr 并设置 m_errorString。
	...
	// 释放 fn 所对应 DLL 返回的结果指针（仅当该 DLL 导出了 ClearMem）。raw 为 nullptr 时安全跳过；老扩展未导出 ClearMem 时不做任何事，兼容其“static 缓冲、调用方无需释放”的旧约定。
