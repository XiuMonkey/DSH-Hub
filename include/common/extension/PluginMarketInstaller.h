...
// ------------------------------------------------------------------
// PluginMarketInstaller.h
// ------------------------------------------------------------------
// 插件市场包（dshmarket）缺失时的自动安装逻辑，从 PluginsManager 中抽出：
//   - 生成 pnpm shim（系统 PATH 里没有 pnpm 时用本地 node 跑 pnpm.cjs）；
//   - 以 DSH_HOME / PATH 环境变量启动 pnpm add dshmarket；
//   - 装完后把 dshmarket 写进 profile 的 package.json（dependencies + dsh.profile.bundles）。
// 只对外抛出“开始 / 输出行 / 结束”三种通知，界面文案由 UI 决定。
// ------------------------------------------------------------------
