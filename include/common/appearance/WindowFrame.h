...
namespace WindowFrame
{
	...
	// ------------------------------------------------------------------
	// 半透明遮罩的显隐（遮罩怎么画在 popups.qss 的 #windowScrim 里）。
	// 一个宿主窗口只留一层：设置 / 插件市场 / 扩展管理 / 工具过滤共用；owner 按
	// 调用方记名，最后一个 release 的才真正隐藏 —— 所以 show/hide 必须成对。
	// 隐藏时末尾补一次宿主同步重绘，让遮罩和弹窗落在同一帧。
	void hideOverlay(QWidget* host, const void* owner);

	// 铺遮罩 + 把弹窗居中并显示，背靠背完成，中间不插任何工作。
	//
	// 为什么必须合成一个函数：遮罩是宿主的子控件（同步重绘后立刻上屏），弹窗是独立
	// 顶层窗口 —— 只有两步在同一个事件循环轮次里完成，合成器才会放进同一帧。中间
	// 夹进耗时工作（建控件、拉数据）就会出现"遮罩先出、弹窗后到"。
	// 所以调用方的准备工作要在调本函数**之前**做完。
