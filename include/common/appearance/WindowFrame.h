...
// 与控件的约定（写在控件侧，这里只做判定）：标题栏控件 objectName 必须是 windowTitleBar；窗口按钮控件必须带动态属性 dshWindowControl=true —— 否则鼠标落在按钮上会被系统当成标题栏拖动，按钮永远收不到点击。
...
namespace WindowFrame
{
	...
	// 边框的收尾组合动作（顺序固定在这里）：切可见面圆角描边状态 + 补最大化内容边距，返回是否贴屏幕边缘（调用方据此刷新最大化/还原图标）；surface 传窗口的可见面（主窗口即 centralWidget()）。
	...
	// 半透明遮罩的显隐（怎么画在 popups.qss 的 #windowScrim）：一个宿主只留一层，设置/插件市场/扩展管理/工具过滤共用，owner 按调用方记名，最后一个 release 的才真正隐藏 —— show/hide 必须成对；隐藏时末尾补一次宿主同步重绘，让遮罩和弹窗落在同一帧。
