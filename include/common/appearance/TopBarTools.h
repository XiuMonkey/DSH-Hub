...
// 顶栏工具的「功能半」（不含任何控件）：ToolsFilter 读写 DSH 里 ToolsFilterPlugin 插件暴露的 /api/tools-filter —— GET ?session=<会话 id> 取该会话可见的工具目录（名字 + 描述 + 参数格式）与过滤状态，POST { sessionId, config: { FilterList, DropGuidance } } 整份写回该会话的 ToolsFilterConfig.json。
// 陷阱：隐藏只能靠 IsVisible="False"（把条目从 ToolsList 里删掉表达不了隐藏）；目录的 IsExpanded 是筛选语义（"False" = 整组隐藏，见插件的 compileFilterList），与界面上的展开/收起无关，平时只做原样带回（唯一例外：目录条目工具行底部的「折叠/可见」开关会改写它所属目录的这个值，见 TopBar.cpp 的 groupHiddenChanged 信号）。
// 为什么这个头文件里没有 Q_OBJECT：它在 vcxproj 里登记为普通 ClInclude（不跑 moc），所以异步结果一律用 std::function 回调返回（UI 半在 include/TopBar.h，QtMoc ✔）；请求全部走 QNetworkAccessManager 的完成回调，主线程只做收发、从不等待，因此不会卡界面。
