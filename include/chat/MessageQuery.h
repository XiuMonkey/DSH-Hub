...
// 0.1.5 历史模型：首屏优先取 mux 上 session/follow 的首帧快照（cursor/records/hasMore，只播种最近 kSeedEventCap 条），
// 上翻走一元 RPC session/page（必带 throughSeq；beforeSeq=当前最早 seq 且为排他上界；maxMessages 数的是消息不是事件）。
...
public:
	...
	// 把离屏构建好的“更早一页”插到本列表/布局顶部（layoutIndex 起）并接管 older 的所有权（调用后 older 不可再用）
...
public:
	...
	// 恢复缓存时接管分页状态（游标/最早 seq/内容最新 seq + m_seeded）：快照不比缓存内容新就不重建，保住“缓存命中即秒开”
...
private:
	...
	// “已到历史顶部”：只由服务端回包驱动；勿用 m_history->hasMore() 门控（该值在部分流程与服务端不符，会误报“没有更多”）
