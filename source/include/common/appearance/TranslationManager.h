...
// .ts 里只写 <message id="key"> + <translation>，id 是查表主键，<source> 不参与查表也不在运行期使用。
// 陷阱：源码里没有任何自然语言文案，每条文案用全局唯一 key 引用，因此所有语言（含出厂默认的中文）都
// 需要语言包、没有"某个语言免翻译"的例外；查不到译文时兜底是 key 本身，界面会显示 topbar_settings
// 这样的代号 —— 这是刻意保留的：缺哪条一眼可见。
// 语言包从 exe 同目录 translations/ 读，装不上再退回 qrc 内置（内置包启动时释放到外部目录、存在则不
...
// .ts 清单里的一条记录，用于"就地换文案"（见 Translation::snapshotWidgetTexts）；ID-based 模式下
// 查表主键是 id 而不是原文，所以这里必须带 id。
...
{
	// 出厂默认语言：缺语言包时回退到它；key 化之后没有任何语言"免翻译"，它同样需要语言包
	// （源码里的兜底是 key 本身，不是中文）。
	...
	// 就地换文案，让"构造时写死文案"的界面也能免重启切语言：换 translator 之前用当前 translator 建立
	// 「界面此刻显示的文案 -> id」映射，换好之后遍历所有控件替换命中项；用 arg() 拼出来的动态文案与
	// 已渲染进 HTML 的历史消息不在覆盖范围内，需各自的重建路径。
