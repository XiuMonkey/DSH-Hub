...
// 会话相关“功能逻辑”的统一入口（无 Qt Widget 依赖）：拉取并解析 session.list、触发侧栏重建与自动选中、
// session.create、标题回填、清空 DSH home 会话数据。回调都在主线程触发（DshApiClient 的回调本来就在主线程）。
...
{
public:
	// 拉取 session.list 并写入 catalog（不含工作区，后者由 workspace/follow 提供）；成功回调
	// onReady(autoSelectSessionId)，失败回调 onError 且此时不写会话列表。
	static void refreshSessions(DshApiClient* api, SessionCatalog* catalog,
	...
	static void createSession(DshApiClient* api, const QString& workspaceId,
