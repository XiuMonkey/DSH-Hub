...
namespace UiStage
{
	// 抢台。owner 必须等于扩展的安装目录名（宿主的 ClientExtension 会在 attachHost() 之前用可选槽 setHostIdentity 把它推给插件），宿主靠它做所有权校验与“卸载时强制收台”。成功返回空舞台（已挂进窗口、已是当前可见面）；失败返回 nullptr：已被别人占着（同一时刻只允许一个 owner，绝不静默顶掉）、owner 为空 / 非 GUI 线程。同一 owner 重复调用是幂等的。
	...
	// 在所有顶层窗口上把 owner 占的台收掉，返回收掉的数量。必须有这条兜底：插件崩了或忘了在自己的 detachHost() 里调 ExternalReleaseStage 的话，宿主控件树里会留着一个 vtable 指向已解映射内存的控件，unload() 之后碰一下就崩。
