...
// .ext 扩展包（zip）的解析与安装，按 regulation.json5 里的 Type 分流：无 Type / 其它值 → 工具扩展（包内有 Function[] 与 AttachedPlugin/，装进服务端扩展目录 extensions.json + cordis.patch.yml，由 DllCaller 在 Worker 线程按 JSON 协议调用）；ClientExtension[Debug] → 客户端扩展（包里只需 regulation + 一个 dll，装进 <exe>/clientExtensions/<Name>/，由 ClientExtension 在 GUI 线程用 QPluginLoader 装载，详见 ClientExtension.h）。
// 两者只共用 .ext 这层壳：载荷、宿主、线程、登记位置都不同。标准 .ext 布局（工具扩展）：AttachedPlugin/{package.json,index.js} + main.dll + regulation.json5。
...
public:
	struct LoadedExtension
	{
		...
		// true = 走了客户端扩展路线：调用方应在 GUI 线程调 ClientExtension::loadOne(pluginName, dllPath)，且不要碰 extensions.json / cordis.patch.yml / 服务端重启。
