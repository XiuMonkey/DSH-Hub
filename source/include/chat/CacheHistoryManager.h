...
public:
	...
	// 记录缓存建立时的分页/新鲜度快照；判"缓存还能不能用"必须靠游标而非内容条数
	// （首屏已改为 session/follow 快照）
	...
	void cacheOrDiscardCurrentSession(const QString& sessionId, MessageQuery* messages, QVBoxLayout* layout);
