...
// 与界面无关的 HTTP / JSON / 配置形状都在 include/TopBarTools.h 的 ToolsFilter 里。
// 目录行**默认全折叠**，展开只是"这一次翻看"的显示状态；配置里的 IsExpanded（"False" = 整组隐藏）是另一回事。
...
// 一个目录分组：表头按钮点击展开/收回名下工具行（纯显示、不写配置），底部「折叠/可见」开关才写该目录的 IsExpanded（折叠 = 整组对模型隐藏）
...
private:
	...
	// 展开状态必须自己记：窗口未显示时子控件的 isVisible() 一律为 false，用 m_body->isVisible() 反推会吞掉"第一次点击"
	...
	// 写配置 IsExpanded，是本条目里唯一改筛选语义的控件；类型只用基类，因为头文件看不见 TopBar.cpp 匿名命名空间里的实现类
	QAbstractButton* m_groupToggle = nullptr;
};

// 工具过滤窗口：按目录分组的勾选列表，勾上 = 该工具会出现在发给模型的清单里；隐藏只影响提示词（省 token），工具仍可被调用。
...
private:
	...
	// 被收起的目录名，默认全折叠；判断"新会话"用 m_collapsedSeedSession 而不是 m_sessionId（setContext 会先覆盖它，比较永远相等）
...
public:
	...
	// 交出顶栏横向布局（不转移所有权；调用方插进去的控件随后归顶栏所有）。跨 DLL 只能走 obj->qt_metacast(IID) 或接口虚函数 vtable：
	// ⚠️ 别改成 dynamic_cast（Itanium ABI 下跨模块静默返回 nullptr），也别 qobject_cast<TopBar*>（要 TopBar::staticMetaObject，宿主 exe 的外部符号且零导出 ⇒ 插件 DLL 链接期 LNK2019）。
...
private:
	...
	// **类级成员**（不能做成构造里的局部变量：外部要经 GetLayout() 拿它）；顺序：标题 | stretch | 已挂的外部控件 | 工具按钮
