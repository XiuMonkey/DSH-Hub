...
// 小灰字的原始数据：对应服务端 sessionStats / tokenUsage 两块投影（session/list 每行的 projections.values），字段语义同官方 Web 端 StatsLine；每字段在首个事件前都是 0，所以 0 = "还没有"，不是"真的是 0"。
struct SessionUsageStats
{
	int turns = 0;            // 轮：至少闭合了一个 step 的不同轮次
	int steps = 0;            // 步：step/end 计数（失败/取消的步也算）
	qint64 llmMs = 0;         // LLM 用时：step/start -> assistant/message
	qint64 toolMs = 0;        // 工具用时：tool/call -> tool/result 按 callId 配对
...
// 卡片下方那行小灰字：固定 14px 行高（输入区留白里那 14px 就是给它的），始终显示（与官方"无内容整行不渲染"是有意差异）。
