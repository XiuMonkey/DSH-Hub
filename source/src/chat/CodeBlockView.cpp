	...
	: QTextEdit(parent)
{
	...
	// 滚动条是在 QTextEdit 基类构造里建好的，那时 objectName 还没设，QStyleSheetStyle 会把
	// "匹配不到 #codeBlockView QScrollBar"缓存下来，之后 setObjectName 不会触发重新匹配 ——
	// 滚动条会一直是原生（老式）样式。所以设完 objectName 后强制重新解析一次。
	...
	connect(document(), &QTextDocument::contentsChanged, this, &CodeBlockView::updateHeightToContent);
}

void CodeBlockView::setCodeHtml(const QString& html)
{
	// 用 <pre> 包装：保留缩进与换行（Qt 富文本只在 <pre> 内保留空白），字体与控件自身的等宽设置一致
...
{
	// 不依赖惰性的 documentLayout()->documentSize()（见历史 bug：会算成单行），按"行数 × 行高"
	// 计算：NoWrap + 源码换行结构下，每行源码占一行。
