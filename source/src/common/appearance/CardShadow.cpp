...
namespace CardShadow
{
	namespace
	{
		// 缓存预算（字节，按设备像素计）：高 DPI 下每条位图大 4 倍、能放下的条数就变少，这是内存上限该有的行为
		// （要保住命中率就按 dpr² 调大）。
		constexpr qint64 kCacheBudget = 24 * 1024 * 1024;
		// 单张超过此大小不进缓存：一张巨图会顶掉整个预算，把常用的小阴影全挤出去。8MB 为实测值 —— 长回复气泡约
		// 880×1250 逻辑像素、位图约 4.3MB，旧 4MB 上限下每帧重建需 3.08ms，是命中（0.175ms）的 17 倍。
		...
		// 高斯尾 Q(x)=0.5*erfc(x/√2)：真实"模糊"阴影离形状边界 d 处的强度，d=0 处正好 0.5（模糊阴影的半高位置）；
		// 旧版把边界处配成累积满峰值，于是贴着卡片是一条实边、观感像"又细又重的一条描边"。
			...
			const QColor& color, qreal dpr)
		{
			...
			// 与原生 CSS 同构的两层阴影（紧层贴卡片、宽层铺开，见原版 --dsw-shadow-lv2 的
			// `0 4px 12px 2%, 0 2px 8px 4%`）：单层高斯看着"薄"，两层叠起来才有"东西压在上面"的厚度；
			// 两层总 alpha 仍等于色板那一个值，色值依旧只来自 JSON。
			...
			// 由目标曲线反解每环透明度：距离 d 的像素被所有 grow>d 的环覆盖，累积 alpha(d) = 1 - Π(1-a_i)，
			// 令其等于 target(d) 得 a_d = 1 - (1-target(d)) / (1-target(d+1))。
		...
		const QColor& color, qreal devicePixelRatio, const QMargins& padOverride)
	{
		...
		if (it == c.end()) {
			...
			// 必须按设备像素记账：位图是 imageSize*dpr，只算逻辑尺寸会漏掉 dpr² —— 150%/200% 屏上记账值只有真实
			// 内存的 1/2.25 或 1/4，"24MB 预算"实际吃到 ~96MB，单张上限也会误判（逻辑 1.5MB 的气泡真实 5.8MB）。
