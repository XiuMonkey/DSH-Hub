...
namespace
{
	// 一个宿主窗口的架空状态；用哈希表而非单例字段：切主题时"新窗口先建、旧窗口下一轮事件循环才析构"，两者会短暂共存。
	struct StageState
	{
	...
	// 只按宿主窗口指针记；窗口销毁时靠 destroyed 连接把自己的条目摘掉，否则这张表会随切主题逐次长大。
	...
	// 宿主浮层（设置 / 插件市场 / 扩展管理 / 工具过滤 / 模型选择菜单…）是**独立顶层窗口**，不跟着客户区一起下线，不收掉就会直接飘在扩展画面上；刻意用 close() 而不是 hide()：close() 才会走到 closeEvent → PopupWindow::closed() → WindowFrame::hideOverlay，遮罩那一层才会被正确 release（遮罩按 owner 记名，只有对应调用方 release 才真正隐藏）；只 hide() 会把遮罩留在原地、直接盖住舞台。
	void closeHostPopups(QMainWindow* host)
	{
		...
		for (QWidget* w : tops) {
			if (!w || w == host || !w->isWindow())
				continue;
			// 只碰"挂在宿主名下的"顶层窗口，别人的一概不动（常驻 Tooltip 单例无父、不在其列，它靠鼠标移开自己收）
		...
		// 兜底：万一还有遮罩挂着（例如关闭被某处拦下），显式收一次 —— 宁可少一层遮罩，也不能让它盖在舞台上面。
	...
	{
		// 初始化遮罩是 buildUi() 里 raise() 起来的，而抢台发生在它之后（ClientExtension::loadAll() 在 DSHHub 构造末尾），新挂进去的客户区会盖住它 —— 把遮罩重新顶上来，"初始化中"那张卡才看得见。
...
namespace UiStage
{
	QWidget* acquire(QMainWindow* host, const QString& owner)
	{
		...
		// 原生客户区下线：**重新挂回窗口当普通子控件再 hide()** —— 不要让它变成无父顶层窗口，那样它会丢掉窗口样式表的继承（QSS 是按顶层窗口挂的，见 ThemeManager::applyToWindow），还会混进 QApplication::topLevelWidgets()，而 ThemeManager::reload() 正是按那张表刷样式的。
		...
		// 舞台常驻、只切可见面，还台时不销毁它 —— 避免"延迟销毁落到插件 unload() 之后"（那时插件的 vtable 已解映射，析构一次就是崩）。
		if (!st.stage) {
			st.stage = new QWidget(host);
			st.stage->setObjectName(QStringLiteral("uiStage"));
			// 刻意**不叫** dshhubCentral、也不设 WA_StyledBackground：宿主的 QSS 全是 "#dshhubCentral <后代>" 的选择器（例如 defaults.qss 的通用 QPushButton / QLineEdit 规则），舞台作为原生客户区的**兄弟**才能天然免疫这套级联 —— 这也是"完全自绘"不需要扩展去对抗宿主样式表的原因。
		}
		// 顺序要紧：先 setCentralWidget（会 reparent 进窗口）再 show()；反过来在没有父的前提下 show() 会让它短暂变成顶层窗口。
