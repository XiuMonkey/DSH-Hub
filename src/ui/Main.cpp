...
namespace
{
	...
	// 顶栏阴影规格：**与侧栏同一档**（两块面板紧挨着，阴影不一致一眼就看出来）。
	// 单独写一份是为了能只调顶栏 —— 真要改数值，两边一起改才协调。
	const CardShadow::Spec kTopBarShadow = { 8, 2, 12 };
	// 顶栏卡片与会话列之间的视觉间距。改造前来自 rightColumnLayout 的 spacing(8)，
	// 现在改由阴影外壳的下留白承担、外壳外面的布局间距归零 —— 间距与占用的总高都不变。
...
void DSHHub::buildUi()
{
	...
	// 侧栏顶部与顶部栏对齐：阴影外壳默认上留白 = blur - dy（当前 6px），
	// 会把侧栏卡片上沿压低一截。把上留白置 0、下留白保持不变，
	// 卡片下沿位置不变，相当于侧栏向上长高这段，顶边与顶部栏齐平。
	...
	// 间距归零：顶栏外面套了阴影外壳，外壳自带下留白 —— 两者的和才是
	// 「顶栏卡片 → 会话列」的距离。下留白给 8px、这里归零，视觉间距与
	// 占用的总高都与改造前一致（同下面 kContentBottomInset 的处理）。
	...
	// 顶栏外面套阴影外壳，档位与侧栏相同（两块面板挨着，不一致会看出来）。
	...
	// 卡片在外壳里的落位：上留白置 0，卡片上沿与侧栏卡片上沿齐平（同侧栏外壳的处理）；
	// 下留白留 kTopBarShadowGap 给阴影落脚，正好等于改造前的行间距。
	// 注意上留白为 0 意味着卡片上方的阴影没有落脚处、不会被画出来 —— 侧栏同理，
	// 两块都贴着内容区上沿，这样才一致。
	...
	// 左右边距扣掉外壳的阴影留白：外壳向外多出的那圈正好落在留白里，
	// 卡片本体的左右边缘仍在原来的 16px 上（与消息内容、输入卡片对齐）。
