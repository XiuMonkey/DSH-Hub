                                     ...
                                     QString *error)
{
    ...
    QString archivePath = extFilePath;
    QString tempZipPath;

    // Expand-Archive only accepts .zip filenames. Extension packages are .ext,
    // so copy to a temporary .zip before extracting when needed.
    const QFileInfo extInfo(extFilePath);
    if (extInfo.suffix().compare(QStringLiteral("zip"), Qt::CaseInsensitive) != 0) {
        tempZipPath = QDir::temp().filePath(
            QStringLiteral("DSHHubExtract_%1.zip")
                .arg(QUuid::createUuid().toString(QUuid::WithoutBraces)));

        if (!QFile::copy(extFilePath, tempZipPath)) {
            m_errorString = QStringLiteral("cannot copy extension to temporary zip: %1").arg(extFilePath);
            if (error)
                *error = m_errorString;
            return false;
        }
    ...
    const QString escapedExt = QString(archivePath).replace(QStringLiteral("'"), QStringLiteral("''"));
    const QString escapedDest = QString(destDir).replace(QStringLiteral("'"), QStringLiteral("''"));
    ...
    process.start(QStringLiteral("powershell"),
                  {
                      ...
                      QStringLiteral("Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::ExtractToDirectory('%1','%2')")
                          .arg(escapedExt, escapedDest)
                  });
    if (!process.waitForStarted(10000)) {
        m_errorString = QStringLiteral("failed to start powershell for extension extraction");
        if (error)
            *error = m_errorString;
        return false;
    }

    // Keep reading stdout/stderr while waiting so a chatty process cannot fill the pipe and hang.
    QElapsedTimer timer;
    timer.start();
    while (process.state() != QProcess::NotRunning && timer.elapsed() < 60000) {
    ...
    if (!tempZipPath.isEmpty())
        QFile::remove(tempZipPath);

    if (!finished) {
        m_errorString = QStringLiteral("timed out extracting extension");
        if (error)
            *error = m_errorString;
        return false;
    }
    ...
    if (exitCode != 0) {
        m_errorString = QStringLiteral("failed to extract extension: %1")
                                    ...
                                    QString *error)
{
    ...
    // Persist the DLL bridge files so DSH Hub can load them after restart.
    // The .ext package layout uses regulation.json5 + main.dll.
    const QString extRoot = profileDir.filePath(QStringLiteral("extensions"))
                                     + QStringLiteral("/") + out->pluginName;
    QDir().mkpath(extRoot);
    if (!out->jsonPath.isEmpty()) {
        const QString destJson = extRoot + QStringLiteral("/regulation.json5");
        QFile::remove(destJson);
        if (!QFile::copy(out->jsonPath, destJson)) {
            m_errorString = QStringLiteral("cannot copy regulation.json5: %1").arg(out->jsonPath);
            if (error)
                *error = m_errorString;
            return false;
        }
    }
    if (!out->dllPath.isEmpty()) {
        const QString destDll = extRoot + QStringLiteral("/main.dll");
        QFile::remove(destDll);
        if (!QFile::copy(out->dllPath, destDll)) {
            m_errorString = QStringLiteral("cannot copy main.dll: %1").arg(out->dllPath);
            if (error)
                *error = m_errorString;
            return false;
        }
