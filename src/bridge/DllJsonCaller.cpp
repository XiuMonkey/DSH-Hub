                             ...
                             QString *errorMessage)
{
    ...
    if (!m_library || !m_library->isLoaded()) {
        ...
        m_errorString = QStringLiteral("DLL not loaded");
        if (errorMessage)
            *errorMessage = m_errorString;
        return false;
    }
                                       ...
                                       QString *error)
{
    ...
    if (fn.returnType == QStringLiteral("string")) {
        ...
        auto *symbol = reinterpret_cast<StringFn>(m_library->resolve(fn.function.toUtf8().constData()));
        if (!symbol) {
            m_errorString = QStringLiteral("cannot resolve function: %1 (%2)")
                                .arg(fn.function, m_library->errorString());
            if (error)
                *error = m_errorString;
            return false;
        }
    ...
    if (fn.returnType == QStringLiteral("void")) {
        auto *symbol = reinterpret_cast<VoidFn>(m_library->resolve(fn.function.toUtf8().constData()));
        if (!symbol) {
            m_errorString = QStringLiteral("cannot resolve function: %1 (%2)")
                                .arg(fn.function, m_library->errorString());
            if (error)
        ...
        auto *symbol = reinterpret_cast<IntFn>(m_library->resolve(fn.function.toUtf8().constData()));
        if (!symbol) {
            m_errorString = QStringLiteral("cannot resolve function: %1 (%2)")
                                .arg(fn.function, m_library->errorString());
            if (error)
                                         ...
                                         QString *error)
{
    ...
    auto *symbol = m_library->resolve(fn.function.toUtf8().constData());
    if (!symbol) {
        m_errorString = QStringLiteral("cannot resolve function: %1 (%2)")
                            .arg(fn.function, m_library->errorString());
        if (error)
