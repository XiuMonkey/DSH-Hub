    ...
    , m_api(new DshApiClient(this))
{
    ...
    // 左侧灰色会话列表
    m_sessionList = new SessionQuery(central);
    ...
    connect(m_sessionList, &SessionQuery::newSessionRequested,
            this, &DSHHub::onNewSessionClicked);
    connect(m_sessionList, &SessionQuery::sessionSelected,
            this, &DSHHub::onSessionSelected);
    connect(m_sessionList, &SessionQuery::clearRequested,
    ...
    connect(m_sessionList, &SessionQuery::settingsRequested,
...
void DSHHub::openSettings()
{
    ...
    m_settingsPopup = new SettingsPopup(this);
    m_settingsPopup->move(geometry().center() - m_settingsPopup->rect().center());
    m_settingsPopup->show();

    connect(m_settingsPopup, &PopupWindow::closed, this, [this]() {
        closeSettings();
    });
}
...
void DSHHub::closeSettings()
{
    ...
    if (m_settingsPopup) {
        disconnect(m_settingsPopup, nullptr, this, nullptr);
        m_settingsPopup->close();
        m_settingsPopup->deleteLater();
...
void DSHHub::startBundledServer()
{
    ...
    // 如果还没复制到客户端目录，则回退到已安装的 DSH Desktop
    if (!QFile::exists(nodePath) || !QFile::exists(dshEntry)) {
        const QString appData = QProcessEnvironment::systemEnvironment().value(QStringLiteral("APPDATA"));
        nodePath = QStringLiteral("D:/Program Files/DSH Desktop/resources/app/node_modules/node/bin/node.exe");
        entryPath = QStringLiteral("D:/Program Files/DSH Desktop/resources/harness-node-entry.mjs");
        dshEntry = QStringLiteral("D:/Program Files/DSH Desktop/resources/app/node_modules/@deepseek-ai/dsh/lib/bin.js");
        cwd = appData + QStringLiteral("/DSH Hub/launch-root");
        m_connectionSource = QStringLiteral("已安装的 DSH Desktop");
        dshHome = appData + QStringLiteral("/DSH Hub/harness");
    }
    m_dshHome = dshHome;
...
void DSHHub::onNewSessionClicked()
{
    m_sessionList->requestNewSession(
        m_api,
        [this]() {
            hideLoadingIndicator();
            cacheCurrentMessages();
        },
        [this](const QString &newSessionId) {
        ...
        [this](const QString &code, const QString &message) {
            if (m_messages) {
                m_messages->addSystemMessage(
                    QStringLiteral("新建对话失败: %1 %2").arg(code, message),
                    m_messagesLayout);
            }
        });
}
