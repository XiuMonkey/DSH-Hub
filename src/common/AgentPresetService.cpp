...
namespace AgentPresetService
{
	...
	void persistDefault(
		...
		const std::function<void(const DshApiClient::RpcError& error)>& onError)
	{
		// 空 id 不能写：服务端不校验 id 是否存在，写进去会让之后每次建会话都
		// 以 agent-preset/not-found 失败（见头文件里的说明）。
		if (presetId.isEmpty()) {
			if (onError) {
		...
		// settings/update 是"把 patch 浅并进该命名空间的 user 段"，
		// 回包是该命名空间（已脱敏）的新视图 —— 这里只需要"写成功"这个事实，
		// 不解析回包：下一次 agentPresets/list 会带出新的 isDefault。
		api->callMethod(
			QLatin1String(kSettingsUpdateMethod),
