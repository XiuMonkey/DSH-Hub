...
# 所以本脚本不再"从源码生成 .ts"，而是做三件事：
#
#   1) lupdate 抽一份**临时**清单，只为对比：源码新增/删除了哪些 key。
#      绝不能拿它更新正式 .ts —— lupdate 会清空手写的 <source>、并把译文回标成
#      type="unfinished"（lrelease 默认跳过这种条目），那样编出来的包会缺文案，
...
#     <message id="xxx"><source>中文原文</source><translation>译文</translation></message>
#   再跑本脚本；漏了哪条它会列出来。
#
...
#   .\tools\release-translations.ps1 -CheckOnly      # 只做 key 一致性校验
#
...
# 与 TranslationManager.cpp 的 kBuiltinPacks / resources\DSHHub.qrc 保持一致：
...
$code = Invoke-Quiet $lupdate @("-recursive", (Join-Path $root "src"), (Join-Path $root "include"), "-ts", $tmpTs)
...
if ($missing.Count -gt 0) {
	...
	throw "请先在 translations\dshhub_*.ts 里补上这些 id 的 <source> 与 <translation>"
}
