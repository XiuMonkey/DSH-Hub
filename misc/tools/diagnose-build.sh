...
#   "<PortableGit>/bin/bash.exe" misc/tools/diagnose-build.sh
set -u
...
INT="$ROOT/source/DSH Hub/x64/Release"
...
if [ -n "$running" ]; then
	...
	echo "  → 处理：关闭它，或直接跑 misc/tools/rebuild.sh（它会先关再编）。"
else
	echo "  ✓ 没有实例在跑。"
fi
...
echo "=================================================="
newest_src=$(ls -t "$ROOT"/source/src/core/*.cpp "$ROOT"/source/src/chat/*.cpp "$ROOT"/source/include/*.h 2>/dev/null | head -1)
...
total_err=0
for f in "$ROOT"/source/src/core/DSHHub.cpp "$ROOT"/source/src/core/MessageHost.cpp; do
	name=$(basename "$f")
	...
	out=$(cl.exe /nologo /Zs /std:c++17 /Zc:__cplusplus /permissive- /EHsc /utf-8 \
		/I"$ROOT/source/include" /I"$QT" /I"$QT/QtCore" /I"$QT/QtGui" \
