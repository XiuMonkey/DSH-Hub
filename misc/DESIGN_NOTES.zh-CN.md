...
> **路径基准（2026-09 目录重组后）**：本文里的相对路径按所在组补前缀 ——
> `src/`、`include/`、`CMake/` 在 **`source/`** 下；原 `unit_test/` 现在是 **`tests/`**；
> `tools/` 在 **`misc/`** 下；`resources/`、`translations/`、`StyleExample/` 在 **`assets/`** 下。
> 例：`src/chat/MessageQuery.cpp` → `source/src/chat/MessageQuery.cpp`。
> 写成 `<exe>/resources/...` 的是**运行期**路径，不在此列。
...
- `.ts` 的形状契约：只写 `<message id="key">` + `<translation>`；id 是查表主键，`<source>` **不参与查表**、也不在运行期使用（写了也会被 `misc/tools/release-translations.ps1` 删掉），原文由 `zh_CN` 的 `<translation>` 承担。
...
- `.ts` 的正式文件**不维护 `<source>`**，写了也会被 `misc/tools/release-translations.ps1` 删掉；原文由 `zh_CN` 的 `<translation>` 承担。
